...
test('one instruction destination preserves old text, global drafts, commands and project overrides', async ({ page, request }) => {
  ...
  await expect(page.getByRole('checkbox', { name: 'Use automatically' })).not.toBeChecked();
  ...
  // The global text is not a row of the project's list; it is in the prompt the project's chats start with.
  await page.getByRole('button', { name: 'View prompt', exact: true }).click();
  await expect(page.getByTestId('library-preview')).toContainText('Global guidance for every provider');
  await page.screenshot({ path: join(results, 'unified-project.png'), animations: 'disabled' });
  await page.keyboard.press('Escape');
  await page.getByRole('radio', { name: 'Commands', exact: true }).click();
  await page.getByTestId('library-item-release-audit').getByRole('button').first().click();
  ...
  await page.getByTestId('library-editor').getByRole('button', { name: 'Save', exact: true }).click();
  await expect(page.getByTestId('library-item-release-audit')).toContainText('Customized');
...
test('project rules are read-only, explain applicability and preserve older settings', async ({ page, request }) => {
  ...
  await expect(summary.getByRole('heading', { name: 'Active', exact: true })).toBeVisible();
  await expect(summary.getByTestId('library-item-applies')).toContainText('Active');
  await expect(summary.getByTestId('library-item-previous-override')).toContainText('Customized');
  await summary.getByTestId('library-item-previous-override').getByRole('button').first().click();
  await expect(detail).toContainText('This project has a saved override.');
  for (const name of ['Add instruction', 'Import', 'Customize', 'Edit', 'Enable', 'Disable', 'Reset', 'Delete']) await expect(page.getByRole('button', { name, exact: true })).toHaveCount(0);
  await expect(page.getByRole('heading', { name: 'Additional rules', exact: true })).toHaveCount(0);
  await expect(summary.getByRole('heading', { name: 'Inactive · 1', exact: true })).toBeVisible();
  await expect(summary.getByTestId('library-item-missing-file')).toContainText('Inactive');
  await summary.getByTestId('library-item-missing-file').getByRole('button').first().click();
  await expect(detail).toContainText('not-here.json');
  await expect(summary.getByRole('heading', { name: 'Legacy project rules · 1', exact: true })).toBeVisible();
  ...
  await summary.getByTestId('library-item-applies').getByRole('button').first().click();
  await detail.getByRole('link', { name: 'Edit global instructions' }).click();
  await expect(page.getByRole('button', { name: 'Add instruction', exact: true })).toBeVisible();
  await page.getByTestId('library-item-applies').getByRole('button').first().click();
  ...
  await page.goto(`/project?id=${project.id}&settings=library`);
  await page.getByTestId('library-item-applies').getByRole('button').first().click();
  await expect(detail.getByText('Edited globally', { exact: true })).toBeVisible();
  expect((await (await request.get(url)).json()).library).toEqual(before);
});
