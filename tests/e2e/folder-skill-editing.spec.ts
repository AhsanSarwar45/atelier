...
test('project folder edits and global customizations remain in their own scopes', async ({ page, request }) => {
  ...
  try {
    ...
    await page.getByRole('radio', { name: 'Skills', exact: true }).click();
    await page.getByTestId('library-item-edit-local').getByRole('button').first().click();
    await page.getByTestId('library-detail').getByRole('button', { name: 'Edit', exact: true }).click();
    await page.getByLabel('Item content', { exact: true }).fill('Project command content.');
    await page.getByRole('checkbox', { name: 'Use automatically' }).uncheck();
    await page.getByTestId('library-editor').getByRole('button', { name: 'Save', exact: true }).click();
    ...
    await page.getByRole('radio', { name: 'Skills', exact: true }).click();
    await page.getByTestId('library-item-edit-global').getByRole('button').first().click();
...
test('provider settings link to the single shared output-style selector', async ({ page }) => {
  ...
  await expect(page.getByRole('combobox', { name: 'Selected output style' })).toBeVisible();
});
