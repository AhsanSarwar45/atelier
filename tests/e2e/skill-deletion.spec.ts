...
test('delete a skill folder in its own scope without touching another project', async ({ page, request }) => {
  ...
  try {
    ...
    await expect(detail).toHaveAttribute('data-item', 'delete-shared');
    await expect(detail.getByRole('button', { name: 'Delete', exact: true })).toHaveCount(0);
    await local.getByRole('button').first().click();
    await page.getByTestId('library-detail').getByRole('button', { name: 'Delete', exact: true }).click();
    ...
    await textRow.getByRole('button').first().click();
    await page.getByTestId('library-detail').getByRole('button', { name: 'Delete', exact: true }).click();
    await dialog.getByRole('button', { name: 'Delete skill', exact: true }).click();
    ...
    await commandRow.getByRole('button').first().click();
    await page.getByTestId('library-detail').getByRole('button', { name: 'Delete', exact: true }).click();
