...
/// The name the node runtime wears as a file on this computer.
#[cfg(windows)]
...
/// Where the chat runtime is, preferring the copy the release carried beside
/// the program over any the reader installed themselves.
///
/// A release now ships its own node next to the atelier binary, so a computer
/// with no node of its own still runs the chat. It is resolved by the program's
/// own location rather than a search of the system: a copy the machine starts
/// at login was handed none of the reader's places, and the runtime it carried
/// has to be found without them (bw-oesd.2). The order is
///
///   1. `ATELIER_NODE`, named by hand -- somebody testing the bundle or pinning
///      a runtime of their own choosing.
///   2. the node beside the resolved program file -- the release archive and
///      Homebrew layouts, where atelier and node are siblings in one folder
///      (`current_exe` is canonicalized so Homebrew's `bin` symlink is followed
///      to the real file in `libexec`).
///   3. the node in a neighbouring `libexec` -- the nix layout, where the
///      program is in `bin` and its runtime one folder over.
///   4. only then the system search, so a developer running `cargo run` with no
///      bundle, and a reader who installed their own node, are both still served.
pub fn find_runtime() -> Option<PathBuf> {
    if let Some(named) = std::env::var_os("ATELIER_NODE") {
        let named = PathBuf::from(named);
        if runnable(&named) {
            tracing::info!("Using ATELIER_NODE runtime at: {}", named.display());
            return Some(named);
        }
        tracing::warn!("ATELIER_NODE names {} but nothing runnable is there", named.display());
    }
    if let Ok(exe) = std::env::current_exe() {
        let exe = exe.canonicalize().unwrap_or(exe);
        if let Some(found) = bundled_runtime(&exe) {
            tracing::info!("Using carried runtime at: {}", found.display());
            return Some(found);
        }
    }
    find_node()
...
/// The carried runtime beside a program at `exe`, if the release laid one down.
///
/// Split out from [`find_runtime`] so the two layouts can be proved against a
/// directory a test owns rather than wherever this process happens to run from.
fn bundled_runtime(exe: &Path) -> Option<PathBuf> {
    let dir = exe.parent()?;
    let candidates = [
        // Siblings in one folder: the release archive, and Homebrew's `libexec`.
        dir.join(NODE_FILE),
        // The program in `bin` and its runtime one folder over: the nix layout.
        dir.join("..").join("libexec").join(NODE_FILE),
    ];
...
mod tests {
    ...
    /// The runtime the release carried sits beside the program, and the search
    /// finds it there before ever looking down the system's list of places --
    /// which is the whole point on a computer that has no node of its own.
    #[test]
    fn a_runtime_beside_the_program_is_chosen_over_the_system_path() {
        let home = tempfile::tempdir().expect("a directory of our own");
        // The layout of the release archive and of Homebrew's libexec: the
        // program and its runtime are siblings in one folder.
        let bin = home.path().join("Cellar");
        std::fs::create_dir_all(&bin).expect("a place for the program");
        let exe = bin.join("atelier");
        install(&bin, "atelier");
        let node = install(&bin, NODE_FILE);
        assert_eq!(
            bundled_runtime(&exe),
            Some(node),
            "the node beside the program is the one carried, and it is taken first"
        );
    }

    /// The nix layout puts the program in `bin` and its runtime one folder over
    /// in `libexec`; the search reaches across for it.
    #[test]
    fn a_runtime_one_folder_over_in_libexec_is_found() {
        let out = tempfile::tempdir().expect("a directory of our own");
        let bin = out.path().join("bin");
        let libexec = out.path().join("libexec");
        std::fs::create_dir_all(&bin).expect("a bin folder");
        std::fs::create_dir_all(&libexec).expect("a libexec folder");
        let exe = bin.join("atelier");
        install(&bin, "atelier");
        let node = install(&libexec, NODE_FILE);
        assert_eq!(
            bundled_runtime(&exe).map(|p| std::fs::canonicalize(p).unwrap()),
            Some(std::fs::canonicalize(node).unwrap()),
            "the node one folder over is found when none sits beside the program"
        );
    }

    /// With no runtime carried in either place, the bundled search says so, so
    /// the caller falls through to the reader's own installed node.
    #[test]
    fn no_carried_runtime_means_the_search_falls_through_to_the_system() {
        let home = tempfile::tempdir().expect("a directory of our own");
        ...
        assert_eq!(
            bundled_runtime(&exe),
            None,
            "nothing was carried, so the bundled search finds nothing and the system is asked"
        );
    }
}
