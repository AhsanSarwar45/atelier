...
mod tests {
    ...
    /// The python this replaces, if this computer has one to run it with.
    fn what_python_writes(event: &Value) -> Option<Value> {
        use std::io::Write;
        let script = Path::new(env!("CARGO_MANIFEST_DIR"))
            .parent()?
            .join("workbench/hooks/session-doing.py");
        if !script.is_file() {
            return None;
        }
    ...
    #[test]
    fn it_writes_the_very_same_line_the_python_it_replaces_wrote() {
        // The screens were built and proved against the script's output. This
        // is the same line by construction; the check is that it stays so
        // wherever there is a python to ask. A computer without one — which is
        // the whole reason this exists — has the cases above instead.
        let events = [
            json!({"hook_event_name": "PreCompact", "session_id": CHAT, "trigger": "manual"}),
            json!({"hook_event_name": "Notification", "session_id": CHAT,
                   "notification_type": "permission_prompt",
                   "message": "Claude needs your permission to use Bash"}),
        ];
        for event in events {
            let Some(mut theirs) = what_python_writes(&event) else {
                eprintln!("no python on this computer, so the two were not compared");
                return;
            };
            let home = tempfile::tempdir().expect("a folder");
            let sessions = home.path().join("sessions");
            fire(&sessions, event.clone());
            let mut ours = line(&sessions).expect("a line");
            // The one field that must differ: they were written a moment apart.
            ours["since"] = json!(0);
            theirs["since"] = json!(0);
            assert_eq!(ours, theirs, "the two disagree about {event}");
        }
    }
}
