...
pub fn run(name: &str, rest: &[String]) -> Option<Result<i32, String>> {
    let tool = match name {
        "board/job" | "board/land" | "board/reconcile" | "board/status" | "board/cleanup"
        | "board/reclaim" | "checks" | "review" => name,
        _ => return None,
    };
...
fn asks_for_help(rest: &[String]) -> bool {
    rest.iter()
        .any(|word| matches!(word.as_str(), "--help" | "-h" | "--schema"))
}
...
pub(crate) fn root() -> Result<PathBuf, String> {
    let program =
        crate::routes::find_git().ok_or_else(|| crate::routes::GIT_MISSING.to_string())?;
    let output = Command::new(program)
        .args(["rev-parse", "--show-toplevel"])
        .output()
        .map_err(|error| format!("could not ask Git for the project root: {error}"))?;
    if !output.status.success() {
        return Err("this is not a Git project".into());
    }
    Ok(PathBuf::from(
...
pub(crate) fn flag(rest: &[String], name: &str) -> Option<String> {
    rest.iter()
        ...
        .or_else(|| {
            rest.iter()
                .find_map(|word| word.strip_prefix(&format!("{name}=")).map(str::to_string))
        })
}
...
pub(crate) fn flags(rest: &[String], name: &str) -> Vec<String> {
    ...
    while let Some(word) = words.next() {
        ...
        } else if let Some(value) = word.strip_prefix(&format!("{name}=")) {
...
fn created_id(output: &str) -> Result<String, String> {
    let value: Value = serde_json::from_str(output)
        .map_err(|error| format!("bd returned unreadable JSON: {error}"))?;
    value["id"]
        .as_str()
        .or_else(|| {
            value
        ...
        .map(str::to_string)
        .ok_or_else(|| "bd created a card but returned no id".into())
}
...
fn make_item(
    ...
    let (title, done) = item
        .split_once('|')
        .ok_or_else(|| format!("work item must be '<what>|<done>': {item}"))?;
    if title.trim().is_empty() || done.trim().is_empty() {
        return Err("work item title and acceptance must not be empty".into());
    }
...
fn chosen_spine(rest: &[String]) -> Vec<String> {
    let requested: Vec<String> = flag(rest, "--steps")
        .unwrap_or_default()
        .split(',')
        .map(str::trim)
    ...
    ["ground", "design"]
        .into_iter()
        .filter(|step| requested.iter().any(|wanted| wanted == step))
        .chain(std::iter::once("work"))
        .chain(
            ["checks", "benchmark", "review", "record"]
                .into_iter()
                .filter(|step| *step == "checks" || requested.iter().any(|wanted| wanted == step)),
        )
        .chain(std::iter::once("land"))
        .map(str::to_string)
        .collect()
}
...
pub(crate) fn metadata(root: &Path, id: &str, entries: &[(&str, String)]) -> Result<(), String> {
    ...
    for (key, value) in entries {
        args.extend(["--set-metadata".into(), format!("{key}={value}")]);
    }
    bd(root, &args).map(|_| ())
}
...
fn job(rest: &[String]) -> Result<i32, String> {
    ...
    if action == "under" {
        let parent = rest
            .get(1)
            .ok_or_else(|| "board/job under needs a goal id".to_string())?;
        let parent_value: Value = serde_json::from_str(&bd(
            &root,
            &["show".into(), parent.clone(), "--json".into()],
        )?)
        .map_err(|error| error.to_string())?;
        let card = parent_value
            .as_array()
            .and_then(|rows| rows.first())
            .unwrap_or(&parent_value);
        let labels: Vec<&str> = card["labels"]
            .as_array()
            .into_iter()
            .flatten()
        ...
        let area = labels
            .iter()
            .find_map(|label| label.strip_prefix("area:"))
            .unwrap_or("board");
        let kind = labels
            .iter()
            .find_map(|label| label.strip_prefix("kind:"))
            .unwrap_or("feature");
        let priority = card["priority"].as_i64().unwrap_or(2).to_string();
        let items = flags(rest, "--do");
        if items.is_empty() {
            return Err("board/job under needs at least one --do '<what>|<done>'".into());
        }
        for item in items {
            println!(
                "{}",
                make_item(&root, parent, &item, area, kind, &priority)?
            );
        }
    ...
    if action == "cancel" {
        let id = rest
            .get(1)
            .ok_or_else(|| "board/job cancel needs an id".to_string())?;
        let reason = flag(rest, "--reason")
            .filter(|reason| !reason.trim().is_empty())
            .ok_or_else(|| {
                "board/job cancel needs --reason explaining why the work is being dropped"
                    .to_string()
            })?;
    ...
    if action == "upgrade" {
        let id = rest
            .get(1)
            .ok_or_else(|| "board/job upgrade needs a card id".to_string())?;
        let existing = card(&root, id)?;
        if existing["status"].as_str() == Some("closed") {
            return Err(format!("{id} is closed"));
        }
        let what = flag(rest, "--what")
            .unwrap_or_else(|| existing["title"].as_str().unwrap_or(id).to_string());
        let done = flag(rest, "--done")
            .or_else(|| existing["acceptance_criteria"].as_str().map(str::to_string))
            .ok_or_else(|| "--done must not be empty".to_string())?;
        let area = flag(rest, "--area").unwrap_or_else(|| {
            labels(&existing)
                .iter()
                .find_map(|label| label.strip_prefix("area:"))
                .unwrap_or("board")
        ...
        let kind = flag(rest, "--kind").unwrap_or_else(|| {
            labels(&existing)
                .iter()
                .find_map(|label| label.strip_prefix("kind:"))
                .unwrap_or("feature")
        ...
        metadata(
            ...
            &[
                ...
                (
                    "judge",
                    flag(rest, "--judge").unwrap_or_else(|| "agent".into()),
                ),
            ],
        )?;
        ...
        for item in items {
            println!("{}", make_item(&root, id, &item, &area, &kind, &priority)?);
        }
        advance_goal(&root, id)?;
        println!("{id}");
        return Ok(0);
    }
    if !matches!(action, "new" | "epic") {
        return Err("usage: atelier tool board/job new|upgrade|epic|under|cancel ...".into());
    }
    ...
    if done.trim().is_empty() {
        return Err("--done must not be empty".into());
    }
    ...
    let priority = flag(rest, "--priority")
        .or_else(|| flag(rest, "-p"))
    ...
    if let Some(parent) = flag(rest, "--parent") {
        args.extend(["--parent".into(), parent]);
    }
...
fn cancel_tree(root: &Path, id: &str, reason: &str) -> Result<(), String> {
    ...
    fn collect(
        ...
        if !visiting.insert(id.into()) {
            return Err(format!("Cycle in cancellation scope at {id}"));
        }
        ...
        {
            return Err(format!(
                "{id} belongs to another actor; its scope cannot be cancelled by this session"
            ));
        }
        if row["status"] == "manager_review" {
            return Err(format!("{id} needs the manager's scope decision"));
        }
        for child in children(root, id)? {
            if child["status"] != "closed" {
                if let Some(child_id) = child["id"].as_str() {
                    collect(root, child_id, actor, visiting, out)?;
                }
            }
        }
        visiting.remove(id);
    ...
    collect(
        root,
        id,
        &crate::board_landing::actor(root)?,
    ...
    for id in ids {
        bd(
            ...
            &[
                ...
                "--force".into(),
                "--set-metadata".into(),
                "status_derived=false".into(),
                "--append-notes".into(),
                format!("Cancelled scope: {reason}"),
            ],
        )?;
    }
    Ok(())
...
pub(crate) fn card(root: &Path, id: &str) -> Result<Value, String> {
    let value: Value =
        serde_json::from_str(&bd(root, &["show".into(), id.into(), "--json".into()])?)
            .map_err(|error| error.to_string())?;
    Ok(value
        .as_array()
        .and_then(|rows| rows.first())
...
pub(crate) fn advance_all(root: &Path) {
    if let Err(error) =
        crate::board_landing::recover(root).and_then(|_| reconcile_parents_for_touch(root))
    {
...
pub(crate) fn main_copy(root: &Path, branch: &str) -> Result<PathBuf, String> {
    ...
    for line in listing.lines() {
        ...
        if line == format!("branch refs/heads/{branch}") {
            return path.ok_or_else(|| "landing worktree has no path".into());
        }
    }
    Err(format!(
        "no worktree has the landing branch {branch} checked out"
    ))
}
...
pub(crate) fn manifest(root: &Path) -> Result<crate::project_manifest::ProjectManifest, String> {
    let data =
        crate::identity::data_dir().ok_or_else(|| "Atelier has no data directory".to_string())?;
    crate::project_manifest::locate(root, &data)
        .or_else(|| {
            crate::project_manifest::locate(&crate::board_landing::common_root(root), &data)
        })
        .map(|found| found.manifest)
        .ok_or_else(|| {
            "This project has no Atelier settings. Run `atelier init` first.".to_string()
        })
}
...
fn changed(root: &Path, trunk: &str) -> Vec<String> {
    git(root, &["diff", "--name-only", &format!("{trunk}...HEAD")])
        .unwrap_or_default()
        .lines()
        .map(str::to_string)
    ...
    for line in output.lines() {
        if let Some(at) = line.find("test result:") {
            ...
            for pair in words.windows(2) {
                if pair[1].starts_with("passed") {
                    passed += pair[0].parse::<u64>().unwrap_or(0);
                }
                if pair[1].starts_with("failed") {
                    failed += pair[0].parse::<u64>().unwrap_or(0);
                }
            }
        }
    }
...
fn recorded_token(name: &str, said: &str) -> Result<String, String> {
    ...
    let (passed, failed) = plain.split_once('/').ok_or_else(|| {
        format!("--record {name}= must say PASSED, FAILED, or PASSED/FAILED counts")
    })?;
    let passed: u64 = passed
        .trim()
        .parse()
        .map_err(|_| "recorded counts must be integers".to_string())?;
    let failed: u64 = failed
        .trim()
        .parse()
        .map_err(|_| "recorded counts must be integers".to_string())?;
    Ok(format!(
...
pub(crate) fn checks(rest: &[String]) -> Result<i32, String> {
    ...
    let trunk = if settings.git.completed_work_branch.is_empty() {
        landing_name(&root)
    } else {
        settings.git.completed_work_branch.clone()
    };
    ...
    let selected: Vec<_> = settings
        ...
        .iter()
        .filter(|suite| {
            all || suite.paths.is_empty()
                || files
                    .iter()
                    .any(|file| suite.paths.iter().any(|path| file.starts_with(path)))
        })
        .collect();
    if dry {
        println!("{} file(s) changed against {trunk}", files.len());
        for suite in &selected {
            println!("would run {}: `{}`", suite.name, suite.command);
        }
        if selected.is_empty() {
            println!("no declared suite matches the changed paths");
        }
        return Ok(0);
    }
    if !git(&root, &["status", "--porcelain", "--untracked-files=no"])?.is_empty() {
        return Err(
            "Commit the tracked changes before recording checks for the landing tree.".into(),
        );
    }
    ...
    if !recorded.is_empty() {
        for result in recorded {
            let (name, said) = result
                .split_once('=')
                .ok_or_else(|| "--record must be NAME=PASSED or NAME=FAILED".to_string())?;
            if !settings
                .verification
                .commands
                .iter()
                .any(|suite| suite.name == name)
            {
                return Err(format!(
                    "--record names {name}, which this project does not declare"
                ));
            }
            let token = recorded_token(name, said)?;
            if token.contains("=FAILED") {
                failure = Some(name.to_string());
            }
            tokens.push(token);
        }
    } else {
        if selected.is_empty() {
            return Err("No declared suite matches the changed paths. Use --all to run every suite, or record evidence on the card explicitly.".into());
        }
        for suite in selected {
    ...
    {
        return Err(
            "The committed tree changed during checks; no passing evidence recorded".into(),
        );
    }
    ...
    if let Some(card) = card {
        ...
        {
            return Err(format!(
                "{card} belongs to another actor; record checks as the claiming session"
            ));
        }
        bd(
            &root,
        ...
        metadata(
            ...
            card,
            &[
                ("checks_tree", tree.clone()),
                (
                    "checks_passed",
                    (failure.is_none() && all_required).to_string(),
                ),
                (
                    "checks_suites",
                    serde_json::to_string(&suite_names).unwrap(),
                ),
            ],
        )?;
...
fn bounded_output(command: &mut Command, said: &[u8]) -> Result<std::process::Output, String> {
    ...
    let mut pipe = child
        .stdin
        .take()
        .ok_or("could not reach the reader's input")?;
    let said = said.to_vec();
    let writing = std::thread::spawn(move || {
        let _ = pipe.write_all(&said);
    });
    ...
    let status = loop {
        ...
        if heartbeat.elapsed() >= Duration::from_secs(30) {
            eprintln!(
                "External review still running ({} seconds)",
        ...
        if started.elapsed() > Duration::from_secs(600) {
            ...
            let _ = child.wait();
            return Err(
                "TIMEOUT: external review exceeded ten minutes; no passing proof recorded".into(),
            );
        }
...
fn review(rest: &[String]) -> Result<i32, String> {
    ...
    let root = root()?;
    if manifest(&root)?.review.external_review == "never" {
        return Err("This project's policy disables external review".into());
    }
    let asked_card = card(&root, asked)?;
    let id = if labels(&asked_card).contains(&"step:review") {
        labels(&asked_card)
            .iter()
            .find_map(|label| label.strip_prefix("of:"))
            .ok_or_else(|| format!("review step {asked} names no job"))?
            .to_string()
    } else {
        asked.clone()
    };
    ...
    let commits: Vec<String> = git(&root, &["log", &format!("{trunk}..HEAD"), "--format=%H"])?
        .lines()
        .map(str::to_string)
        .collect();
    if commits.is_empty() {
        return Err(format!("{id} has no unlanded commits to review"));
    }
    let mut change = String::new();
    for sha in commits.iter().rev() {
        change.push_str(&git(
            &root,
            &["show", "--format=commit %H%n%s", "--stat", "--patch", sha],
        )?);
        change.push('\n');
        if change.len() > 300_000 {
            let mut end = 300_000;
            while !change.is_char_boundary(end) {
                end -= 1;
            }
            change.truncate(end);
    ...
    if git(&root, &["rev-parse", "HEAD^{tree}"])? != reviewed_tree {
        return Err("The tree changed while preparing review".into());
    }
    ...
    if provider == "claude" {
        ...
        return Err("Review provider must be claude or codex".into());
    }
    command.current_dir(&root).env_remove("ATELIER_BYPASS");
    let evidence = crate::board_landing::common_root(&root)
        .join(".git/atelier-reviews")
        .join(format!("{id}-{head}"));
    ...
    let note = format!(
        "external review via {provider} (exit {}):\n\n{}",
    ...
    let parsed = serde_json::from_str::<Value>(verdict.trim())
        .or_else(|_| {
    ...
    let parsed = if provider == "claude" {
        claude_verdict(&parsed)?
    } else {
        parsed
    };
    std::fs::write(
        evidence.join("verdict.json"),
        serde_json::to_vec_pretty(&parsed).map_err(|e| e.to_string())?,
    )
    .map_err(|e| e.to_string())?;
    let findings = parsed["findings"]
        .as_array()
        .ok_or_else(|| "review JSON has no findings array".to_string())?;
    ...
    metadata(
        ...
        &id,
        &[
            ("review_tree", reviewed_tree),
            ("review_passed", passed.to_string()),
            ("reviewed_commits", commits.join(",")),
        ],
    )?;
    if !passed {
        return Ok(1);
...
fn claude_verdict(envelope: &Value) -> Result<Value, String> {
    if envelope["is_error"] == true {
        return Err("REVIEWER_ERROR: reviewer reported failure".into());
    }
    ...
    let text = envelope["result"]
        .as_str()
        .ok_or("REVIEWER_ERROR: missing review verdict")?
        .trim();
    let text = text
        .strip_prefix("```json")
        ...
        .trim();
    let value: Value =
        serde_json::from_str(text).map_err(|e| format!("REVIEWER_ERROR: invalid verdict: {e}"))?;
    if !matches!(value["verdict"].as_str(), Some("PASS" | "NEEDS_WORK"))
...
mod tests {
    ...
    fn native_machinery_a_prompt_too_long_for_an_argument_still_reaches_the_reader() {
        ...
        assert!(
            long.len() > 131_072,
            "the case only bites above MAX_ARG_STRLEN"
        );
    ...
    fn native_machinery_the_codex_review_worker_names_itself_the_agents_own() {
        ...
        let at = args
            .iter()
            .position(|arg| *arg == "--thread-source")
            .expect("the codex review worker must name its thread source");
        let kind = args[at + 1];
        assert_eq!(
            args.last(),
            Some(&"-"),
            "the prompt is still read from stdin"
        );
        assert_eq!(
    ...
    fn native_machinery_review_accepts_json_envelopes_but_not_prose_or_errors() {
        ...
        assert_eq!(
            claude_verdict(&serde_json::json!({"structured_output":result})).unwrap(),
            result
        );
        assert_eq!(
            claude_verdict(&serde_json::json!({"result":format!("```json\n{result}\n```")}))
                .unwrap(),
            result
        );
        assert!(claude_verdict(&serde_json::json!({"result":"Looks good"})).is_err());
        assert!(
            claude_verdict(&serde_json::json!({"is_error":true,"structured_output":result}))
                .is_err()
        );
    }
    ...
    fn native_machinery_commit_subjects_name_exact_cards_only() {
        assert!(subject_names(
            "bw-one.12: finish native machinery",
            "bw-one.12"
        ));
        assert!(!subject_names("bw-one.123: a different card", "bw-one.12"));
        assert!(!subject_names(
            "mention bw-one.12 only in a body we did not pass",
            "bw-one.1"
        ));
        ...
        assert!(subject_names(
            "bw-uxoe: the chat list opens again",
            "bw-uxoe"
        ));
        assert!(subject_names(
            "fix(bw-uxoe): the chat list opens again",
            "bw-uxoe"
        ));
