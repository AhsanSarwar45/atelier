...
fn no_node_runtime_is_started_extracted_or_downloaded_by_the_server() {
    ...
    assert!(!root.join("../workbench").exists(), "the former Node backend still exists");
    let production = [
    ...
    let forbidden = [
        ...
        "BEADS_WORKBENCH_URL", "Command::new(\"node\")", "Command::new(\"npm\")",
        "Command::new(\"python\")", "Command::new(\"python3\")", "../workbench\"",
    ];
