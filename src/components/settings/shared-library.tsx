...
interface Row { item: Item; source: string; customized: boolean; state: string; evaluation: Evaluation; missing: string[]; folder?: { source: string; directory: string; revision: string } }
  ...
  return <div className="space-y-5" data-testid="shared-library">
    ...
    {answer && <>
      ...
      {projectRules ? <ProjectInstructionSummary rows={answer.resolved.items.filter(row => row.item.kind === 'instruction')} /> : draft ? <section className="min-w-0 space-y-6" aria-label="Library item editor" data-testid="library-editor">
        ...
        {row.folder && <p className="break-all text-xs text-t-secondary">Folder-backed skill. Edit SKILL.md and supporting files in <code>{row.folder.source}</code>. Availability settings live in atelier.json.</p>}
        ...
        {row.source !== 'built-in' && !(row.source === 'project' && !projectPath) && <div className="flex flex-wrap gap-2">
          {(!row.folder || (projectPath && row.source === 'global')) && <Button variant="outline" size="sm" onClick={() => { setEditing(row); setDraft(structuredClone(!projectPath ? answer.library.items.find(i => i.id === row.item.id) ?? row.item : row.item)); }}>{projectPath && row.source === 'global' ? 'Customize' : 'Edit'}</Button>}
          {projectPath && row.source === 'global' ? <><Button variant="ghost" size="sm" disabled={saving} onClick={() => void persist({ ...answer.library, overrides: { ...answer.library.overrides, [row.item.id]: { ...answer.library.overrides[row.item.id], disabled: row.state !== 'disabled' } } })}>{row.state === 'disabled' ? 'Enable here' : 'Disable here'}</Button>{row.customized && <Button variant="ghost" size="sm" disabled={saving} onClick={() => { const library = structuredClone(answer.library); delete library.overrides[row.item.id]; void persist(library); }}>Reset to global</Button>}</> : !row.folder && <Button variant="ghost" size="sm" disabled={saving} onClick={() => { if (window.confirm(`Remove ${row.item.name} from this library? Existing chat snapshots retain their copy.`)) void persist({ ...answer.library, items: answer.library.items.filter(i => i.id !== row.item.id), output_style: answer.library.output_style === row.item.id ? '' : answer.library.output_style }); }}>Remove</Button>}
        </div>}
      </Panel>)}</div>}
