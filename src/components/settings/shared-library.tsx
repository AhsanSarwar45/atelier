...
function Conditions({ value, onChange, depth = 0 }: { value: Condition; onChange: (v: Condition) => void; depth?: number }) {
  ...
  return <Panel tone="frame" className="space-y-2" data-testid="condition-builder">
    <Picker label="Condition" value={value.op} onChange={op => onChange(newCondition(op))} choices={Object.entries(conditionNames).filter(([op]) => depth < 7 || !['all', 'any', 'not', 'within'].includes(op)).map(([value, label]) => ({ value, label }))} />
...
export function SharedLibrary({ projectPath }: { projectPath?: string }) {
  ...
  return <div className="space-y-5" data-testid="shared-library">
    ...
    {answer && <>
      ...
      {!projectPath && <div className="space-y-1 text-sm"><span>Evaluate for project</span><Picker label="Evaluate for project" disabled={!!draft} className="w-full" value={preview} onChange={setPreview} choices={[{ value: '', label: 'Choose a project to explain conditions' }, ...projects.filter(p => !(p.localPath || p.path).startsWith('dolt://')).map(p => ({ value: p.localPath || p.path, label: p.name }))]} /></div>}
      {kind === 'output_style' && <div className="space-y-1 text-sm"><span>Selected output style</span><Picker label="Selected output style" disabled={!!draft || saving} className="w-full" value={answer.library.output_style ?? '__inherit'} onChange={value => void persist({ ...answer.library, output_style: value === '__inherit' ? null : value })} choices={[{ value: '__inherit', label: projectPath ? 'Use global selection' : 'No style selected' }, { value: '', label: 'No shared output style' }, ...answer.resolved.items.filter(r => r.item.kind === 'output_style' && (projectPath || r.source !== 'project')).map(r => ({ value: r.item.id, label: r.item.name }))]} /><span className="block text-xs text-t-muted">One shared style is included at a time. Native provider styles may still apply.</span></div>}
      ...
      {draft ? <Panel tone="frame" className="space-y-4" data-testid="library-editor">
      ...
      </Panel> : <div className="space-y-3">{rows.length === 0 && <Panel tone="frame" className="text-sm text-t-secondary">No {names[kind].toLowerCase()} yet. Add one to share it across providers.</Panel>}{rows.map(row => <Panel key={row.item.id} tone="frame" className="space-y-2" data-testid={`library-item-${row.item.id}`}>
      ...
      <Panel tone="frame" asChild><details><summary className="cursor-pointer text-sm font-medium">Saved session preview</summary><p className="mt-2 break-all text-xs text-t-muted">Revision {answer.resolved.revision}. Instructions and the selected style are included; skills are loaded on use. Current chats retain their connection’s snapshot.</p><pre className="mt-3 max-h-80 overflow-auto whitespace-pre-wrap text-xs">{answer.guidance}</pre></details></Panel>
    </>}
  </div>;
}
