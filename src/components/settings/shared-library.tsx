  ...
  return <div className="space-y-5" data-testid="shared-library">
    ...
    {answer && <>
      ...
      {projectRules ? <ProjectInstructionSummary rows={answer.resolved.items.filter(row => row.item.kind === 'instruction')} /> : draft ? <section className="min-w-0 space-y-6" aria-label="Library item editor" data-testid="library-editor">
        ...
        {row.folder_source && <p className="break-all text-xs text-t-secondary">Folder-backed skill. Edit SKILL.md and supporting files in <code>{row.folder_source}</code>. Availability settings live in atelier.json.</p>}
        ...
        {row.source !== 'built-in' && !(row.source === 'project' && !projectPath) && <div className="flex flex-wrap gap-2">
          {(!row.folder_source || (projectPath && row.source === 'global')) && <Button variant="outline" size="sm" onClick={() => { setEditing(row); setDraft(structuredClone(!projectPath ? answer.library.items.find(i => i.id === row.item.id) ?? row.item : row.item)); }}>{projectPath && row.source === 'global' ? 'Customize' : 'Edit'}</Button>}
