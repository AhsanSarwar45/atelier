...
export function SharedLibrary({ projectPath, projectInstructions }: { projectPath?: string; projectInstructions?: ReactNode }) {
  ...
  return <div className="space-y-5" data-testid="shared-library">
    {!draft && <div><div className="flex items-center gap-2"><BookOpen className="size-5 text-t-muted" /><h2 className="text-lg font-semibold">Agent guidance</h2><Badge>{projectPath ? 'This project' : 'Global'}</Badge></div><p className="mt-2 text-sm text-t-secondary">{projectPath ? 'Create project guidance or customize what this project inherits.' : 'Manage guidance shared by all projects, across every provider.'}</p></div>}
    ...
    {answer && <>
      ...
      <div hidden={!!draft || kind !== 'instruction'} className="space-y-6 border-b border-border/60 pb-6">{projectPath ? projectInstructions : <GeneralInstructions saved={answer.library.general_instructions ?? ''} saving={saving} onSave={text => persist({ ...answer.library, general_instructions: text })} />}{projectPath && answer.resolved.items.find(r => r.item.id === 'atelier-general-instructions') && <details className="text-sm"><summary className="cursor-pointer font-medium">Also inherited: global instructions</summary><p className="mt-2 text-xs text-t-muted">These apply alongside project instructions. Edit them in global Agent guidance.</p><pre className="mt-2 whitespace-pre-wrap break-words text-xs">{answer.resolved.items.find(r => r.item.id === 'atelier-general-instructions')?.item.content}</pre></details>}<div><h3 className="font-semibold">Additional rules</h3><p className="mt-1 text-sm text-t-secondary">Optional named rules, including rules that only apply when conditions match.</p></div></div>
      {!draft && <div className="flex flex-wrap items-start justify-between gap-3"><p className="max-w-sm text-sm text-t-secondary">{category === 'command' ? 'Procedures you run explicitly from the chat composer. Commands are never chosen automatically by the agent.' : descriptions[kind]}</p><div className="flex flex-wrap gap-2"><Button onClick={() => create()}><Plus className="mr-1 size-4" />Add {itemLabel}</Button><Button variant="ghost" onClick={() => void importNative()}>Import native file</Button></div></div>}
