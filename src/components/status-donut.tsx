...
/**
 * The breakdown, as the app's own hover label rather than a panel this file
 * positions itself. It used to hang off the bottom of the donut in a box of
 * its own making, which is how the board came to have three different-looking
 * answers to the same gesture (bw-6wq6.1).
 */
function StatusBreakdown({ beadCounts, total }: { beadCounts: BeadCounts; total: number }) {
  return (
    <div className="whitespace-nowrap">
      <div className="mb-1.5 font-medium text-t-secondary">
...
export function StatusDonut({ beadCounts, size = 40, className, countsLoaded = true }: StatusDonutProps) {
  ...
  return (
    <Tooltip side="bottom" label={<StatusBreakdown beadCounts={beadCounts} total={total} />}>
      <div data-testid="status-donut" className={`relative ${className || ""}`} style={{ width: size, height: size }}>
