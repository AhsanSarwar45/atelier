...
export function MemoryBadge() {
  ...
  const read = useCallback(() => { void fetch(apiUrl('/api/workbench/memory'), { cache: 'no-store' })
    .then(async response => { if (!response.ok) throw new Error(await response.text()); return response.json() as Promise<MemoryReport>; })
  ...
  return <Popover onOpenChange={open => open && read()}>
    <PopoverTrigger asChild><Badge asChild appearance="outline" size="sm" shape="circle" className="hidden shrink-0 md:inline-flex">
      <button type="button" data-testid="memory-badge" aria-label={`Atelier RAM usage: ${memoryWords(report.totalBytes)}`}><MemoryStick className="size-3" aria-hidden="true" /><span>{memoryWords(report.totalBytes)}</span></button>
