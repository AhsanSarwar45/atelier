...
export function isRunning(s: LiveSession): boolean {
  return s.state === 'thinking' || s.state === 'streaming' || s.state === 'waiting_for_agents' || s.state === 'running_tool'
    // Folding itself up is work, and the longest of them: a chat mid-fold that
    // read as not running dropped off the list of what is being worked on for
    // the two minutes it takes (bw-ryh3.1).
    || s.state === 'summarising';
}
...
function absorb(frame: WatchFrame): void {
  ...
  switch (e.type) {
    ...
    case 'session.state': {
      ...
      // session.state is authoritative for live, replay and snapshots alike.
      // Notices never keep a hidden copy of an earlier running state to restore.
      patch(e.sessionId, {
        ...base,
