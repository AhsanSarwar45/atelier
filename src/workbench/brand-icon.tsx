...
export function BrandIcon({ brand, label, className }: { brand: Brand; label?: string; className?: string }) {
  if (brand === 'local') {
    return (
      <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.8" strokeLinecap="round" strokeLinejoin="round" className={cn('size-4 shrink-0', className)} aria-label={label ?? brandName(brand)} role="img">
        <rect x="5" y="5" width="14" height="14" rx="3" />
        <path d="M9 9h6v6H9zM9 2v3m6-3v3M9 19v3m6-3v3M2 9h3m-3 6h3m14-6h3m-3 6h3" />
      </svg>
    );
  }
  const path = brand === 'codex'
  ...
  return (
    <svg viewBox="0 0 24 24" fill="currentColor" className={cn('size-4 shrink-0', className)} aria-label={label ?? brandName(brand)} role="img">
      <path d={path} />
    </svg>
  );
}
