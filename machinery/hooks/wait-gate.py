...
def cases():
    r"""Every command this gate is held to, one per line.

    They are kept in a docstring because shell is not English, and the voice
    check weighs every other string a hook holds as if it were. A `\n` in a
    line stands for a newline.

    refuse      until curl -s -o /dev/null http://127.0.0.1:3018/api/projects; do sleep 2; done
    refuse      while ! grep -q DONE build.log; do sleep 5; done
    refuse      for i in 1 2 3 4 5; do curl -s localhost:3018 && break; sleep 1; done
    refuse      sleep 30; tail -5 build.log; sleep 30; tail -5 build.log
    refuse      npm run build & while ! grep -q Ready dev.log; do sleep 0.5; done
    refuse      curl -s localhost:3018 > a.json; sleep 5; curl -s localhost:3018 > b.json; sleep 5; curl -s localhost:3018 > c.json
    background  until curl -s -o /dev/null http://127.0.0.1:3018/api/projects; do sleep 2; done
    background  while ! grep -q DONE build.log; do sleep 5; done
    allow       sleep 2; npm test
    allow       npm test && sleep 1
    allow       grep -rn asleep machinery/
    allow       python3 -c "import time; time.sleep(1)"
    allow       ssh box 'sleep 1'
    allow       node -e "setTimeout(go, 1000)"
    allow       cat > x.sh <<'EOF'\nwhile true; do sleep 1; done\nEOF
    allow       for f in *.py; do python3 -m compileall $f; done
    allow       git log --oneline | while read line; do echo $line; done
    """
    for line in (cases.__doc__ or "").splitlines():
        verb, _, rest = line.strip().partition(" ")
        if verb in ("refuse", "allow", "background") and rest.strip():
            yield verb, rest.strip().replace("\\n", "\n")
...
def selftest():
    ...
    for verb, command in cases():
        ...
        got = judge("Bash", given)
        if bool(got) != (verb == "refuse"):
            failed.append("%s: wanted %s\n      %s"
                          % (verb, "a refusal" if verb == "refuse" else "no refusal",
                             " ".join(command.split())))

    # A refusal has to quote the agent its own words back, or it is a lecture.
    # One fragment per refused case above, in the order they are written there.
    quoted = (
        "curl -s -o /dev/null http://127.0.0.1:3018/api/projects",
        "grep -q DONE build.log",
        "curl -s localhost:3018",
        "tail -5 build.log",
        "grep -q Ready dev.log",
        "curl -s localhost:3018",
    )
    refused = [command for verb, command in cases() if verb == "refuse"]
    for command, want in zip(refused, quoted):
        ran += 1
        if want not in (judge("Bash", {"command": command}) or ""):
            failed.append("the gate never quoted back what the wait is for\n      %s"
                          % want)
    if judge("Read", {"command": refused[0]}):
        failed.append("a tool that is not Bash was refused")

    if failed:
        for line in failed:
            print("FAILED  " + line)
        return 1
    print("all %d cases pass" % (ran + 1))
    return 0
